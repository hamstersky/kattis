...
/******************************************************************************
 *  Compilation:  javac UF2.java
 *  Execution:    java UF2 < input.txt
...
/**
 * The {@code UF2} class represents a <em>union–find data type</em> (also known
...
public class UF2 {
  ...
  public void move(int p, int q) {
    ...
    if (rootP == rootQ)
      return;
    // Prepare a variable to store p's child node for later unioning
    int child1 = -1;
    // Loop through all nodes
    for (int i = 0; i < parent.length - 1; i++) {
      // Check if the node at i is part of the same tree as p
      if (find(i) == rootP) {
        // Check if the node at i is a child of p
        if (parent[i] == p) {
          // Check if p is not a root
          if (find(p) != p) {
            // Set the parent of the node at i to the parent of p
            parent[i] = parent[p];
            // Check if the moved node is the root of a tree and i is not p
          } else if (find(p) == p && i != p) {
            // Check if the node at i is a child of p
            if (child1 >= 0) {
              parent[i] = i;
              union(i, child1);
            } else {
              // Store child for unioning if parent is p
              child1 = i;
              // Change the root of the node not to be p anymore
    ...
    // Set p's parent to be the root of q's tree
    parent[p] = find(q);
  }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < m; i++) {
      ...
      switch (operation) {
        case 0:
          System.out.println(uf2.find(s) == uf2.find(t) ? "1" : "0");
          break;
        case 1:
          uf2.union(s, t);
          break;
